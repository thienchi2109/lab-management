...
- UI proof cho xem chi tiết, sửa mẫu, confirm xoá, role-based actions và lỗi
  action fail.
...
- `lab-kit-app/lib/sample-metadata/operations.test.ts`
  - RED: `softDeleteSampleMetadata` chưa tồn tại.
  - Green: xoá mềm ghi audit `sample.deleted`, không lưu full metadata nhạy
    cảm, không hard delete.
...
- `lab-kit-app/app/dashboard/samples/actions.test.ts`
  - RED: `deleteSampleMetadataAction` chưa tồn tại.
  - Green: Admin xoá thành công và revalidate `/dashboard/samples`.
  - Green: Editor/Viewer bị từ chối.
  - Green: validation lỗi `sampleId` trả message an toàn.
...
- `lab-kit-app/app/dashboard/samples/_components/sample-metadata-client.test.tsx`
  - RED: chưa có xem chi tiết và confirm xoá.
  - Green: view side sheet mở từ action row.
  - Green: delete action chỉ render cho Admin.
  - Green: confirm dialog giữ lỗi visible khi action fail.
